...
const calculateNutrition = () => {
  const regex = /Calories: (\d+), Protein: (\d+)g, Carbs: (\d+)g, Fat: (\d+)g/g;
  let totalCalories = 0;
  let totalProtein = 0;
  let totalCarbs = 0;
  let totalFat = 0;
  ...
  let match;
  while ((match = regex.exec(mealPlan)) !== null) {
    totalCalories += parseInt(match[1]);
    totalProtein += parseInt(match[2]);
    totalCarbs += parseInt(match[3]);
    totalFat += parseInt(match[4]);
  }
